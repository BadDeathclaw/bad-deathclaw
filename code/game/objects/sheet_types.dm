...
/*	new/datum/stack_recipe("reflector frame", /obj/structure/reflector, 5, time = 25, one_per_turf = 1, on_floor = 1), \*/ //Fuck them reflectors dawg. We have better projectiles now!
	...
	new/datum/stack_recipe("lock", /obj/item/weapon/lock, 2), \
)
...
/obj/item/stack/sheet/mineral/wood/attackby(var/obj/item/weapon/P, var/mob/livivng/carbon/human/user)
	if(istype(P, /obj/item/weapon/lighter))
		var/obj/coster/newC = new /obj/coster/campfire1
		newC.SetLuminosity(6)
		newC.fuel = 20
		newC.loc = src.loc
		del(src)
